...
/*
 * =========================================================================
 *                     OVERCLOCKING MODIFICATION START
 * =========================================================================
 *
 * This section adds support for a kernel boot parameter to control the
 * maximum frequency of the big CPU cluster.
 *
 * The __setup macro registers a function to parse the 'cpu_max_c1='
 * argument from the kernel command line during boot.
 *
 * Default OC is set to 2.621 GHz, but can be overridden.
 * Example usage in bootloader config: cpu_max_c1=2621000
 * By Minhmc2007
 */
static unsigned long arg_cpu_max_c1 = 2621000; /* Default OC Freq in KHz */
static int __init cpufreq_read_cpu_max_c1(char *str)
{
	/* kstrtoul converts the string from the boot argument to a number */
	kstrtoul(str, 0, &arg_cpu_max_c1);
	pr_info("ACME OC: Big Cluster max frequency set to %lu KHz by boot param\n", arg_cpu_max_c1);
	return 1; /* Return 1 to indicate the parameter was handled */
}
__setup("cpu_max_c1=", cpufreq_read_cpu_max_c1);
/*
 * =========================================================================
 *                      OVERCLOCKING MODIFICATION END
 * =========================================================================
 */
					...
					struct device_node *dn)
{
	 ...
	 * =========================================================================
	 *
	 * Section 1/2: Update the max_freq limit.
	 *
	 * This code overrides the maximum frequency limit for the target cluster
	 * (domain->id == 1, the big cores) with the value from the boot parameter.
	 * This tells the cpufreq governor that a higher frequency is available.
	 * This change alone is not enough; we must also add the new OPP.
	 */
	if (domain->id == 1) {
	 ...
	 * =========================================================================
	 *
	 * Section 2/2: Add the new Operating Performance Point (OPP).
	 *
	 * This is the most critical part. It tells the kernel's power management
	 * framework that a new, higher frequency is valid and specifies the
	 * voltage (in microvolts) required to run at that speed. Without this,
	 * the kernel would ignore our higher max_freq limit.
	 *
	 * The voltage is calculated by taking the voltage of the stock highest
	 * frequency and adding a small amount.
	 *
	 * !! WARNING !! The voltage increase is an educated guess. An incorrect
	 * value can cause system instability or, in extreme cases, damage.
	 * This value may need to be adjusted based on testing.
	 */
	/* We are adding 31.25mV, a reasonable starting point for a ~200MHz OC */
	#define OVERCLOCK_VOLTAGE_BOOST_UV 31250

	if (domain->id == 1) { /* Apply only to the big cluster */
		unsigned int stock_max_volt;
		unsigned int overclock_volt;

		/* Get the voltage of the highest stock frequency */
		stock_max_volt = fv_table[domain->table_size - 1].volt;
		overclock_volt = stock_max_volt + OVERCLOCK_VOLTAGE_BOOST_UV;

		pr_info("ACME OC: Adding custom OPP for Big cluster: %lu KHz @ %u uV\n",
			arg_cpu_max_c1, overclock_volt);

		/* Register this new OPP for all CPUs in the cluster */
		for_each_cpu_and(cpu, &domain->cpus, cpu_possible_mask)
			dev_pm_opp_add(get_cpu_device(cpu),
				arg_cpu_max_c1 * 1000,
				overclock_volt);
...
#ifdef CONFIG_SOC_S5E8535
	struct exynos_cpufreq_dm *dm;
	int mif_maxfreq = cal_dfs_get_max_freq(ACPM_DVFS_MIF);
#endif
...
#ifdef CONFIG_SOC_S5E8535
	/* LP5: change dm table index 1 */
	if (mif_maxfreq == 2730000) {
		list_for_each_entry(domain, &domains, list) {
			list_for_each_entry(dm, &domain->dm_list, list) {
				if (dm->c.dm_slave != DM_MIF)
					continue;

				if (!dm->c.support_variable_freq_table)
					continue;

				exynos_dm_change_freq_table(&dm->c, 1);
				dm->cur_table_index = 1;
				pr_info("domain%d : LP5 -> mif_perf table index to 1\n", domain->id);
			}
		}
	}
#endif
