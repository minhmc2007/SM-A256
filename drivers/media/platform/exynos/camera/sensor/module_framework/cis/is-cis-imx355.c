						...
						u32 *target_duration)
{
	...
	/* HACK for Cts: due to short in frame_duration, there will be adjust for frame_duration 
	   from diff between frame_duration and input_exposure_time. */
	adj_frame_duration = (frame_duration - input_exposure_time) * 2;
	if (adj_frame_duration < 0)
		adj_frame_duration = 0;

	dbg_sensor(1, "[%s](vsync cnt = %d) input exp(%d), adj duration(%d), frame duraion(%d), min_frame_us(%d)\n",
			__func__, cis_data->sen_vsync_count, input_exposure_time, adj_frame_duration, frame_duration, cis_data->min_frame_us_time);
	dbg_sensor(1, "[%s](vsync cnt = %d) adj duration(%d), frame duraion(%d), min_frame_us(%d)\n",
			__func__, cis_data->sen_vsync_count, adj_frame_duration, frame_duration, cis_data->min_frame_us_time);

	*target_duration = MAX(frame_duration, cis_data->min_frame_us_time);
	// HACK for Cts: adjust for frame_duration.
	*target_duration += adj_frame_duration;
